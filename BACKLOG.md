...
**DoD:** ✅ CUMPRIDO (2026-07-12, validado no dispositivo pelo Bruno) — registar sessão em modo avião → condições chegam sozinhas ao voltar o Wi-Fi; valores conferidos manualmente contra open-meteo.com para o mesmo dia/hora/coordenadas; testes do matching verdes. Teste de avião tal como desenhado: registo offline → pending → Wi-Fi de volta → listener do netinfo apanhou a transição → worker correu → cartão passou a ok sem tocar em nada (trigger E reload pós-worker provados num gesto). Fase 2 fechada: 8 tarefas, 95 testes; ficam os P1/P2 opcionais (editar sessão — UI; apagar com undo).
